...
SharingQueue<Work>::SharingQueue() : _skip_waiting{false} {}

template <typename Work>
auto SharingQueue<Work>::try_extract(const policy::IPolicy<Work>& policy)
    -> Result {
  ...
  if (work_empty) _all_work_dispatched.notify_all();
