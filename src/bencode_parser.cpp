...
std::unique_ptr<BencodeValue> BencodeParser::decode(
  ...
  if(!tokens.empty()){
    token = tokens[_index];
    if(token == "i"){
      // If the first token is "i", then it's an integer, so we have a fixed length
      if(_index==0 && tokens.size() != 3){
        throw std::invalid_argument("BencodeParser::decode(std::string decoded): invalid string");
      }
      return BencodeParser::decode_int(tokens);
    }else if(std::regex_match(token, reg_string)){
      // If the first tokes is for a string, then it's a string, so we have a fixed length
...
std::unique_ptr<BencodeValue> BencodeParser::decode_int(
  ...
  if(tokens[_index+1] == "-0"){
    throw std::invalid_argument("BencodeParser::decode_int(std::vector<std::string>& encoded): negative zero is not allowed");
  }
...
std::unique_ptr<BencodeValue> BencodeParser::decode_string(
  ...
  if(len<0 || (unsigned int)len != str.size()) {
    throw std::invalid_argument("BencodeParser::bencode_string(std::vector<std::string>& encoded): invalid string length");
  }
...
std::unique_ptr<BencodeValue> BencodeParser::decode_list(
  ...
  // Check al reference but not space for "e"
  if(_index == tokens.size()){
    throw std::invalid_argument("BencodeParser::bencode_list(std::vector<std::string>& encoded): invalid encoded string");
  }
  // Check if the list is closed with "e"
  if(tokens[_index] != "e") {
    throw std::invalid_argument("BencodeParser::bencode_list(std::vector<std::string>& encoded): missing end of the list");
  }
...
std::unique_ptr<BencodeValue> BencodeParser::decode_dict(
  ...
  if(_index == tokens.size()){
    throw std::invalid_argument("BencodeParser::bencode_list(std::vector<std::string>& encoded): invalid encoded stringgggg");
  }
  // Check if the list is closed with "e"
  if(_index < tokens.size() && tokens[_index] != "e") {
    throw std::invalid_argument("BencodeParser::bencode_dict(std::vector<std::string>& encoded): missing end of the dictionary");
  }
